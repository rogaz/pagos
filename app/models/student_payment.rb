class StudentPayment < ActiveRecord::Base
  ...
  def generate_pdf
    ...
    #pdf.stroke_rectangle([x-5 ,y+5], width*3, height*((self.student_charge.student_payments.count)+1))
    ...
    pdf.stroke
=begin
    pdf.text_box "$#{self.amount.to_s}.00" , :at => [x+width, y+height], :width => width, :height => height
    pdf.text_box self.date.strftime('%d %b %Y') , :at => [x+(width*2), y+height], :width => width, :height => height
    #pdf.font 'Helvetica', :style => :normal
    pdf.text self.amount.to_s #, :at => [x+width, y*2], :width => width, :height => height
    pdf.text self.date.strftime('%d %b %Y') #, :at => [x+(width*2), y*2], :width => width, :height => height
=end
    pdf.font 'Helvetica', :style => :normal
    ...
    pdf.text_box 'Total', :at => [x+width-50, y-((height)*(self.student_charge.student_payments.count+1))], :width => width, :height => height
