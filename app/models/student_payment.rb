class StudentPayment < ActiveRecord::Base
  ...
  def get_path
    directory_date = "#{self.student_charge.date.strftime('%d-%m-%y')}-#{self.student_charge.id}"
  ...
  def generate_pdf
    ...
    total -= self.amount
    pdf.text_box 'Adeudo', :at => [x+width-80, y-((height)*(self.student_charge.student_payments.count+1))], :width => width, :height => height
